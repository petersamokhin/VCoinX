const url = require('url'), {VK,} = require('vk-io')
const {VCoinWS, miner, Entit} = require('./core')
const {
  con,
  ccon,
  setColorsM,
...
let {
  ...
  VK_TOKEN,
  GROUP_ID,
} = require('./config/userconfig.json')
let USER_ID = false
let vk = new VK()
let URLWS = false
let {
...
vCoinWS.onReceiveDataEvent(async (place, score) => {
  ...
  setTerminalTitle('VCoinX (id' + USER_ID.toString() + ') > ' + formatScore(vCoinWS.tick, true) + ' cps > ' + 'top ' + place + ' > ' + formatScore(score, true) + ' coins.')
  if (place > 0 && !rl.isQst) {
    if (transferPercent) {
    ...
    if (transferTo && transferTo !== USER_ID && (transferCoins * 1e3 < score || transferCoins * 1e3 >= 9e9) && ((Math.floor(Date.now() / 1000) - transferLastTime) > transferInterval)) {
      try {
        ...
        if (transferCoins * 1e3 >= 9e9) {
          ...
          template = 'Автоматически переведено [' + formatScore(transferCoins, true) + '] коинов с активного аккаунта (@id' + USER_ID + ') на @id' + transferTo
        }
    ...
    if (autoBuy && vCoinWS.tick <= limitCPS && score > 0) {
      for (let i = 0; i < autoBuyItems.length; i++) {
    ...
    if (!hidejunk) {
      con('[' + USER_ID.toString() + '] Позиция в топе: ' + place + '\tКоличество коинов: ' + formatScore(score, true) + '\tСкорость: ' + formatScore(vCoinWS.tick, true) + ' коинов / тик.', 'yellow')
    }
  }
})
...
vCoinWS.onUserLoaded((place, score, items, top, firstTime, tick) => {
  ...
  setTerminalTitle('VCoinX ' + ' (id' + USER_ID.toString() + ') > ' + formatScore(tick, true) + ' cps > ' + 'top ' + place + ' > ' + formatScore(score, true) + ' coins.')
...
async function smartBuyFunction (score) {
  ...
  if (!tempDataUpdate.transactionInProcess && !tempDataUpdate.onBrokenEvent) {
    let names = ['cursor', 'cpu', 'cpu_stack', 'computer', 'server_vk', 'quantum_pc', 'datacenter']
    let count = [1000, 333, 100, 34, 10, 2, 1]
    let speed = [0.001, 0.003, 0.01, 0.03, 0.1, 0.5, 1]
    let prices
    let payback
    let min_payback
    let good
